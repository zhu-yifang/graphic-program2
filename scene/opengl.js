...
function glInit() {
    ...
    _GL.materialSpecular  =     0.25;
    _GL.materialShininess =     0.2;
...
function glEnd() {
	...
	_GL.normals = [];
	for (let i = 0; i < _GL.points.length; i += 9) {
	    const v0 = [_GL.points[i+0],_GL.points[i+1],_GL.points[i+2]];
	    const v1 = [_GL.points[i+3],_GL.points[i+4],_GL.points[i+5]];
	    const v2 = [_GL.points[i+6],_GL.points[i+7],_GL.points[i+8]];
	    const p0 = new point(v0[0],v0[1],v0[2]);
	    const p1 = new point(v1[0],v1[1],v1[2]);
	    const p2 = new point(v2[0],v2[1],v2[2]);
	    const u = p1.minus(p0);
	    const v = p2.minus(p0);
